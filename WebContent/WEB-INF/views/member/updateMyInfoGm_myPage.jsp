...
<section>
		...
		<form id="updateForm" method="post" action="update.me" onsubmit="return checkSubmit();">
			<table>
				<tr id="top_tr">
					<td class="form_title">단체명</td>
					<td><input type="text" name="userName" value="<%=member.getMem_name() %>">
						<input type="hidden" name="userNo" value="<%=member.getMem_no() %>"></td>
					<td></td>
				</tr>
				<tr>
				...
				<tr>
					<td class="form_title">사업자등록번호</td>
					<td><input type="text" name="userRegNo" class="inputRegNo" value="<%=member.getGm_regno() %>" maxlength="12" required></td>
					<td></td>
				</tr>
				<tr >
					<td class="form_title">대표자명</td>
					<td><input type="text" name="userGmName" value="<%=member.getGm_name()%>" required></td>
					<td></td>
				</tr>
				
				<tr>
					<td class="form_title">전화번호</td>
					<td><input type="text" name="userPhone" class="inputPhone" value="<%=member.getMem_phone()%>" required></td>
					<td></td>
				</tr>
				<tr>
				<%
				...
				%>
					...
					<td><input type="text" placeholder="우편번호" name="zoneCode" id="zoneCode" value="<%=addressArr[0] %>"></td>
...
///////////////////////////////////////////////////////////////////
		...
		///////////////////////////////////////////////////////////////////////
		$('.inputRegNo').on('keyup',function(){  // 사업자 등록번호 필터링 함수
			var inputRegNo = $(".inputRegNo").val();
			console.log(inputRegNo);
			var tmp="";
			inputRegNo = inputRegNo.replace(/[^0-9]/g,'');
			
			if(inputRegNo.length<4){ 
				tmp+=inputRegNo;
			}else if(inputRegNo.length<6){
				tmp+=inputRegNo.substr(0,3);
				tmp+='-'; 
				tmp+=inputRegNo.substr(3);
			}else{
				tmp += inputRegNo.substr(0, 3);  
				tmp += '-';
				tmp += inputRegNo.substr(3, 2); 
				tmp += '-';
				tmp += inputRegNo.substr(5); 
			}
			$(".inputRegNo").val(tmp);
		})
		////////////////////////////////////////////////////////////////////////
